...
dishesAndCooks
  .set('"Margarita" pizza', "cook: Viktor")
  .set('"Pepperoni" pizza', "cook: Viktor")
  .set('"Philadelphia" sushi', "cook: Olga")
  .set('"California" sushi', "cook: Olga")
  .set('"Tiramisu" desert', "cook: Dmitrii")
  .set('"Cheesecake" desert', "cook: Dmitrii");
...
const restaurantView = {
  pushOrderButton() {
    ...
    makeOrderButton.addEventListener("click", (event) => {
      ...
      const selectedDishes = document.querySelectorAll(
      ...
      if (clientName === "" || listOfSelectedDihes.length === 0) {
        ...
        firstParagraph.textContent =
          "Incorrect order! Press 'Clear the form' button and refill the form!";
      } else {
  ...
  pushWatchCookButton() {
    ...
    watchCookButton.addEventListener("click", () => {
      ...
      if (listOfSelectedDihes.length === 0) {
        ...
        warningLi.textContent = "Information is not available! Make an order!";
        ul.append(warningLi);
      } else {
  ...
  showOrder() {
    ...
    for (const [key, value] of restaurantModel.listOfOrders) {
      firstParagraph.textContent = `Client ${key.name} has ordered: ${value}`;
    }
  },
  ...
  showAllOrders() {
    ...
    showOrdersButton.addEventListener("click", () => {
      ...
      allOrdersList.forEach((order) => {
        const li = document.createElement("li");
        li.textContent = `Client: ${order.client} has ordered: ${order.orderedDishes}`;
        secondParagraph.append(li);
      });
      showOrdersButton.setAttribute("disabled", "disabled");
    });
  ...
  pushSecondClearButton() {
    const secondClearButton = document.querySelector(
    ...
    secondClearButton.addEventListener("click", () => {
      ...
      const showOrdersButton = document.querySelector(
...
const restaurantController = {
  ...
  getCooksNames(list) {
    ...
    arrayOfChekedDishes.forEach((dish) => {
      restaurantModel.allCooks.forEach(function (value, key) {
        if (dish === key) {
          const result = `Dish ${dish} has been cooked by ${value}.`;
  ...
  getAllOrders() {
    ...
    if (restaurantModel.listOfOrders.size === 0) {
      const warning = "Order list is empty!";
